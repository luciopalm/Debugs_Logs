...
public class InventoryPaperDollUI : MonoBehaviour
{
    ...
    [Header("Character Portrait (Central Image)")]
    ...
    /// <summary>
    /// 🖼️ Procura automaticamente pelo CharacterImageArea no Paper Doll
    /// </summary>
    private void FindCharacterPortraitImage()
    {
        Debug.Log("🔍 Procurando CharacterImageArea...");
        
        // Primeiro, tenta encontrar no GameObject pai
        if (characterPortraitImage == null && transform.parent != null)
        {
            characterPortraitImage = transform.parent.Find("CharacterImageArea")?.GetComponent<Image>();
        }
        
        // Se ainda não encontrou, procura em toda a cena
        if (characterPortraitImage == null)
        {
            var characterAreaGO = GameObject.Find("CharacterImageArea");
            if (characterAreaGO != null)
            {
                characterPortraitImage = characterAreaGO.GetComponent<Image>();
            }
        }
        
        if (characterPortraitImage != null)
        {
            Debug.Log($"✅ CharacterImageArea encontrado: {characterPortraitImage.gameObject.name}");
        }
        else
        {
            Debug.LogError("❌ CharacterImageArea não encontrado! Configure manualmente no Inspector.");
        }
    }

    /// <summary>
    /// 🖼️ Atualiza o sprite do personagem no centro do Paper Doll
    /// </summary>
    private void UpdateCharacterPortrait()
    {
        Debug.Log($"🖼️ UpdateCharacterPortrait() - Início");
        
        if (characterPortraitImage == null)
        {
            Debug.LogWarning("⚠️ characterPortraitImage não configurado. Tentando encontrar...");
            FindCharacterPortraitImage();
            
            if (characterPortraitImage == null)
            {
                Debug.LogError("❌ characterPortraitImage ainda é null! Pulando atualização.");
                return;
            }
        }
        ...
        if (currentCharacter == null)
        {
            Debug.LogWarning("⚠️ currentCharacter é null - limpando portrait");
            characterPortraitImage.sprite = null;
            characterPortraitImage.color = new Color(0.2f, 0.2f, 0.2f, 0.5f);
            return;
        }
        
        Debug.Log($"👤 Personagem atual: {currentCharacter.characterName}");
        Debug.Log($"📸 Portrait sprite: {currentCharacter.portrait?.name ?? "NULL"}");
        
        if (currentCharacter.portrait != null)
        {
            characterPortraitImage.sprite = currentCharacter.portrait;
            characterPortraitImage.color = portraitTintColor;
            Debug.Log($"✅ Portrait atualizado: {currentCharacter.portrait.name}");
        }
        else
        {
            Debug.LogWarning($"⚠️ {currentCharacter.characterName} não tem portrait sprite!");
            characterPortraitImage.sprite = null;
            characterPortraitImage.color = currentCharacter.themeColor * 0.7f;
        }
        
        // Garantir que o GameObject está ativo
        if (!characterPortraitImage.gameObject.activeSelf)
        {
            Debug.LogWarning("⚠️ CharacterImageArea estava inativo - ativando...");
            characterPortraitImage.gameObject.SetActive(true);
        }
        
        Debug.Log($"🖼️ UpdateCharacterPortrait() - Concluído");
    }
    ...
    public bool TryEquipItem(ItemData item)
    {
        ...
        {
            Debug.LogError("❌ Item inválido para equipar");
            return false;
        }
        
        Debug.Log($"🎯 TryEquipItem: {item.itemName} → {item.equipmentSlot}");
        
        // 🔥 USAR MÉTODO SILENCIOSO
        bool success = InventoryManager.Instance.EquipItemSilent(item);
        
        if (success)
        {
            Debug.Log($"✅ {item.itemName} equipado com sucesso");
            
            // ⚡ Atualizar APENAS o slot específico (muito mais rápido!)
            UpdateSpecificSlot(item.equipmentSlot);
            
            // 🔥 DISPARAR EVENTO ÚNICO no final
            InventoryManager.Instance.OnEquipmentChanged?.Invoke();
        }
        else
        {
    ...
    /// <summary>
    /// ⚡ Atualiza APENAS um slot específico (ultra-rápido)
    /// </summary>
    private void UpdateSpecificSlot(ItemData.EquipmentSlot targetSlot)
    {
        if (InventoryManager.Instance == null) return;
        
        foreach (var slot in paperDollSlots)
        {
            if (slot.slotType == targetSlot)
            {
                UpdateSlot(slot);  // Reutiliza o método existente
                Debug.Log($"⚡ Slot {targetSlot} atualizado");
                break;
            }
        }
    }
    ...
    public void UnequipSelectedSlot()
    {
        ...
        // Limpar seleções após desequipar
        Debug.Log($"║  🧹 Limpando seleções após desequipar...");
        ClearAllSelections();
        
        // 🔥 Também limpar seleção na tabela
        if (inventoryUI != null)
        {
