...
public class InventoryItemDetailsUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("[ItemDetailsUI] PartyManager not found! Tentando encontrar...");
            partyManager = FindFirstObjectByType<PartyManager>();
        }
        
        if (partyManager == null)
        {
            Debug.LogError("[ItemDetailsUI] PartyManager realmente não encontrado!");
            return;
        }
        
        Debug.Log($"[ItemDetailsUI] PartyManager encontrado: {partyManager.gameObject.name}");
    ...
    {
        Debug.Log("╔═══════════════════════════════════════════════════════╗");
        Debug.Log("║  📊 UPDATE PARTY MEMBER STATS                        ║");
        Debug.Log("╠═══════════════════════════════════════════════════════╣");
        ...
        {
            Debug.Log("║  ⚠️ Nenhum member ativo - usando defaults");
            Debug.Log("╚═══════════════════════════════════════════════════════╝");
        ...
        Debug.Log($"║  Character.characterName (SO): '{activeMember.characterName}'");
        
        // 🔥🔥🔥 USAR O MÉTODO DO PARTYMANAGER QUE JÁ EXISTE!
        string displayName = "Player"; // fallback
        
        if (partyManager != null)
        {
            displayName = partyManager.GetPlayerNameForCurrentInstance();
            Debug.Log($"║  🔥 PartyManager retornou: '{displayName}'");
        }
        else
        {
            Debug.LogError("║  ❌ PartyManager é NULL!");
            // Fallback direto se partyManager for null
            if (GameDataManager.Instance != null)
            {
                ...
                {
                    displayName = gameData.playerData.playerName;
                }
            }
        }
        
        Debug.Log($"║  Nome final exibido: '{displayName}'");
        Debug.Log("╚═══════════════════════════════════════════════════════╝");
        
        // 🔥🔥🔥 **AQUI ESTÁ A CHAVE: VERIFICAR partyMemberHeader**
        Debug.Log($"🎯 Tentando atualizar UI...");
        Debug.Log($"   partyMemberHeader é null? {partyMemberHeader == null}");
        
        if (partyMemberHeader != null)
        {
            string textoAtual = partyMemberHeader.text;
            Debug.Log($"   Texto atual na UI: '{textoAtual}'");
            Debug.Log($"   Tentando atualizar para: '{displayName.ToUpper()}'");
            
            partyMemberHeader.text = displayName.ToUpper();
            
            string textoDepois = partyMemberHeader.text;
            Debug.Log($"   Texto depois: '{textoDepois}'");
            Debug.Log($"   Atualizou? {(textoAtual != textoDepois ? "✅ SIM" : "❌ NÃO")}");
        }
        else
        {
            Debug.LogError("❌ partyMemberHeader é NULL! Não pode atualizar!");
            
            // Tentar encontrar
            var allTexts = GetComponentsInChildren<TMPro.TMP_Text>();
            Debug.Log($"🔍 Encontrados {allTexts.Length} textos filhos:");
            foreach (var text in allTexts)
            {
                if (text.name.Contains("Party") || text.name.Contains("Member"))
                {
                    Debug.Log($"   ✅ Candidato: {text.name} = '{text.text}'");
                    partyMemberHeader = text; // Auto-atribui
                }
            }
        }
