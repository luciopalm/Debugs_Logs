...
public class InventoryManager : MonoBehaviour
{
    ...
    /// </summary>
    // Em InventoryManager.cs, modificar o método LoadInventoryFromGameData:
    ...
    public void LoadInventoryFromGameData()
    {
        ...
        // 🔥🔥🔥 CORREÇÃO CRÍTICA: Verificar se é um NOVO JOGO
        var gameData = GameDataManager.Instance.GetCurrentGameData();
        if (gameData != null && gameData.isNewGame)
        {
            Debug.Log("[InventoryManager] ⭐ É UM NOVO JOGO - NÃO carregar dados de save!");
            Debug.Log("[InventoryManager] ⭐ Inventário será inicializado VAZIO");
        ...
        if (!hasValidSave)
        {
            Debug.Log("[InventoryManager] No valid save found - starting with empty inventory");
            isLoadedFromSave = false;
        ...
        // 🔥🔥🔥 SÓ CHEGA AQUI SE FOR UM LOAD DE SAVE VÁLIDO
        ...
        Debug.Log($"✅ Inventory loaded from save: {GetUsedSlotCount()}/{inventorySize} slots, {currentCurrency} currency");
    }
   ...
   public bool AddItem(ItemData item, int quantity = 1)
    ...
    // <summary>
    /// 🔥 NOVO: Limpa COMPLETAMENTE o inventário (para trocar de instância)
    /// </summary>
    public void ClearInventory()
    {
        Debug.Log("[InventoryManager] 🧹 Limpando inventário completamente...");
        ...
        // 2. Limpa equipamentos compartilhados
        if (currentEquipment != null)
        {
            ...
            foreach (ItemData.EquipmentSlot slotType in slotTypes)
            {
                ...
                currentEquipment.UnequipItem(slotType);
            }
        }
        
        // 3. Reseta valores
        currentCurrency = 0; // Usando a variável privada, não a propriedade
        currentWeight = 0f;  // Usando a variável privada, não a propriedade
        
        // 4. Notifica UI
        OnInventoryChanged?.Invoke();
        OnEquipmentChanged?.Invoke();
        
        Debug.Log("[InventoryManager] ✅ Inventário limpo!");
    }
    ...
    [ContextMenu("🔍 Diagnóstico: Verificar Estado dos Slots após Load")]
    public void DebugCheckSlotStateAfterLoad()
    {
        Debug.Log("╔══════════════════════════════════════════════════╗");
        Debug.Log("║  🔍 DIAGNÓSTICO PÓS-LOAD - ESTADO DOS SLOTS     ║");
        Debug.Log("╠══════════════════════════════════════════════════╣");
        ...
        for (int i = 0; i < inventorySlots.Count; i++)
        {
            ...
            Debug.Log($"║     • isEquipped: {slot.isEquipped}");
            Debug.Log($"║     • É equipamento: {slot.item.IsEquipment()}");
            Debug.Log($"║     • Slot type: {slot.item.equipmentSlot}");
            
            // Verificar se deveria ter DraggableItem mas não tem
            bool shouldBeDraggable = slot.item.IsEquipment() && !slot.isEquipped;
            
            if (shouldBeDraggable)
            {
                // Aqui você precisaria verificar se a linha da UI tem DraggableItem
                // Isso depende da UI, mas podemos logar
                Debug.Log($"║     • DEVERIA SER ARRASTÁVEL: ✅");
            }
            
            if (slot.isEquipped && slot.item.IsEquipment())
            {
                equippedSlots++;
                Debug.Log($"║     • ⚠️ EQUIPADO - NÃO DEVE SER ARRASTÁVEL");
            }
        ...
        Debug.Log($"║     • Slots vazios: {emptySlots}");
        Debug.Log($"║     • Slots equipados: {equippedSlots}");
        Debug.Log($"║     • Itens que deveriam ser arrastáveis: {totalSlots - emptySlots - equippedSlots}");
        Debug.Log($"╚══════════════════════════════════════════════════╝");
    }
