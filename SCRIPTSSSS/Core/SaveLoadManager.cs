...
public class SaveLoadManager : MonoBehaviour
{
    ...
    IEnumerator StartNewGameWithInstance()
    {
        ...
        if (GameInstanceManager.Instance != null)
        {
            ...
            // ⭐⭐ MODIFICADO: Pega nome do SaveLoadManager
            var details = GetNewGameDetails();
            string gameName = details.gameName;
            ...
            if (GameDataManager.Instance != null)
            {
                // ⭐⭐ NOVO: Atualiza nome do jogador também
                GameDataManager.Instance.UpdatePlayerName(details.playerName);
    ...
    IEnumerator ContinueLastGameInstance()
    {
        ...
        if (GameInstanceManager.Instance != null)
        {
            ...
            // ⭐ Tenta carregar a última instância jogada
            // (Por enquanto, seleciona a primeira disponível)
            var firstInstance = GameInstanceManager.Instance.GetInstanceInfo(1);
            if (firstInstance != null)
            {
                GameInstanceManager.Instance.SelectGameInstance(firstInstance.instanceID);
                
                // Aguarda seleção
                yield return new WaitForSeconds(0.1f);
                
                // Carrega último slot salvo desta instância
                int lastSlot = firstInstance.lastSaveSlot;
                Debug.Log($"📂 Carregando instância {firstInstance.instanceID}, slot {lastSlot}");
            ...
            {
                Debug.LogError("❌ Instância não encontrada!");
                yield return StartCoroutine(StartNewGameWithInstance());
            }
        }
        else
    ...
    public void LoadFromSlot(int slot)
    {
        ...
        // ⭐⭐ CORREÇÃO CRÍTICA: Usar o MESMO método que GameDataManager usa!
        string filePath = ""; // Declaração inicial

        // Tenta obter o caminho VIA GAMEDATAMANAGER (sistema correto)
        if (GameDataManager.Instance != null)
        {
            try
            {
                // Acessa o método PRIVADO GetSaveFilePath do GameDataManager
                ...
                {
                    // Fallback manual
                    Debug.LogError("❌ Método GetSaveFilePath não encontrado no GameDataManager!");
        ...
        // ⭐⭐ Carrega via GameDataManager (método padrão)
        bool loaded = GameDataManager.Instance.LoadGame(slot);
