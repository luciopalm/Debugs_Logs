...
public class InventoryManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Only add starting items if inventory is empty AND not loaded from save
        bool hasItems = inventorySlots.Any(slot => !slot.IsEmpty);
        
        // DEBUG TEMPORÁRIO
        Debug.Log($"╔══════════════════════════════════╗");
        Debug.Log($"║ DEBUG Start() - InventoryManager");
        Debug.Log($"╠══════════════════════════════════╣");
        Debug.Log($"║ hasItems: {hasItems}");
        Debug.Log($"║ isLoadedFromSave: {isLoadedFromSave}");
        Debug.Log($"║ startingItems.Length: {startingItems.Length}");
        Debug.Log($"║ startingCurrency: {startingCurrency}");
        Debug.Log($"╚══════════════════════════════════╝");
        
        if (!hasItems && !isLoadedFromSave && startingItems.Length > 0)
        {
            Debug.Log("✅ Adding starting items and currency...");
            ...
            bool currencyAdded = AddCurrency(startingCurrency,false);
            Debug.Log($"AddCurrency({startingCurrency}) returned: {currencyAdded}");
        }
        else
        {
            Debug.Log("⚠️ SKIPPING starting items/currency");
        }
    }
